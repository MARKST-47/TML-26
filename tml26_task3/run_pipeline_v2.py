"""
TML Task 3 — Adversarial Robustness Training (v2)
Team: atml_team034
Approach: TRADES + AWP with ResNet18, data augmentation, checkpointing
"""
...
AWP_GAMMA = 0.01     # AWP perturbation scale
PGD_STEPS = 7        # PGD steps for TRADES (7 is sufficient, saves ~30% vs 10)
PGD_STEP_SIZE = 2/255
PGD_EPSILON = 8/255
CHECKPOINT_EVERY = 10
VAL_SPLIT = 2000     # Hold out 2k samples for local validation
...
# Data augmentation (applied on-the-fly on GPU for speed)
...
# ──────────────────────────────────────────────
# Model — ResNet18 adapted for 32x32
# ──────────────────────────────────────────────
...
def evaluate_pgd(model, loader, epsilon=8/255, step_size=2/255, steps=20):
    """PGD-20 attack for local robustness evaluation."""
    model.eval()
    correct, total = 0, 0
    for x, y in loader:
        x, y = x.to(device), y.to(device)
...
print(f"\n{'='*70}", flush=True)
print(f"Starting TRADES + AWP training | ResNet18 | {EPOCHS} epochs", flush=True)
print(f"PGD: eps={PGD_EPSILON:.4f}, steps={PGD_STEPS}, step_size={PGD_STEP_SIZE:.4f}", flush=True)
print(f"BETA={BETA}, AWP_GAMMA={AWP_GAMMA}", flush=True)
print(f"{'='*70}\n", flush=True)
...
for epoch in range(1, EPOCHS + 1):
    ...
    # Evaluate + checkpoint periodically
    if epoch % CHECKPOINT_EVERY == 0 or epoch == EPOCHS:
        ...
        if score > best_score:
            best_score = score
            best_path = os.path.join(SCRIPT_DIR, "model_best.pt")
            torch.save(model.state_dict(), best_path)
            print(f"  ★ New best! Score={score:.4f} saved to model_best.pt", flush=True)
...
print(f"Final model → model.pt | Best model → model_best.pt", flush=True)
