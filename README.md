...
The objective is to forge invisible watermarks onto clean target images such that they deceive an unknown server side detector while maintaining high perceptual fidelity. We are provided with a dataset containing 200 clean target images and multiple sets of watermarked source images across 8 distinct watermark families (WM1​ to WM8​). The core challenge is in navigating the competitive trade-off between detection alignment (Sdet​) and structural preservation (Sqlt​) to maximize a balanced final score (Sfinal ​= Sdet ​× Sqlt​) under strict, non-linear distortion constraints.
